...
public sealed class EveningDigestWorker : BackgroundService
{
    private readonly IServiceScopeFactory _serviceScopeFactory;
    private readonly ILogger<EveningDigestWorker> _logger;

    public EveningDigestWorker(
        IServiceScopeFactory serviceScopeFactory,
        ILogger<EveningDigestWorker> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var nextRunUtc = ResolveNextRunUtc(now, 20);
            ...
            try
            {
                await ProcessDigestAsync(NotificationService.EveningDigestBatchGroup, stoppingToken);
            }
            ...
            {
                _logger.LogError(exception, "Evening digest worker failed.");
            }
        }
    }
    ...
    private static string BuildDigestBody(IReadOnlyCollection<Domain.Entities.Notification> notifications)
    {
        ...
        return $"You have {notifications.Count} updates this evening: {preview}";
    }
}
